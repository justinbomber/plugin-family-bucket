...
var Popup = () => {
    ...
    advanced: () => {
      ...
      chrome.runtime.sendMessage({message: 'popup.advanced'}, () => {
        if (chrome.runtime.lastError) {
          state.advancedError = '無法開啟進階設定，請重試。'
          m.redraw()
        }
      })
    }
  ...
  function scheduleInitRetry() {
    initAttempts += 1
    var delay = 80 + Math.floor(Math.random() * 71)
    setTimeout(function () {
      chrome.runtime.sendMessage({message: 'popup'}, init)
    }, delay)
  }
  ...
  var init = (res) => {
    ...
    if ((res.pending || !res.compilers || !res.compilers.length) &&
    ...
    state.settings = res.settings || {theme: 'light'}
  ...
  var render = () =>
    m('#popup',
      ...
      (state.advancedError
        ? m('p.m-advanced-error', state.advancedError)
        : null
      )
    )
